...
if __name__ == '__main__':
    ...
    current_y = update_current_y([ 0 for i in range(problem.wg) ], circuits, xs, ys)
    print(current_y)
    ...
    for i in range(1, problem.n):
        ...
        # Then, we check if placing that circuit on that x it overflows the
        # board's width
        ...
        xs.append(x)
        ys.append(y)
    # We construct the solution instance so that we can draw everything
